...
if (!fs.existsSync(FOLDERS_ROOT)) fs.mkdirSync(FOLDERS_ROOT, { recursive: true });
if (!fs.existsSync(PRODUCER_FOLDER)) {
  fs.mkdirSync(PRODUCER_FOLDER, { recursive: true });
  console.log(`[PRODUCER ${PRODUCER_ID}] Created local folder: ${PRODUCER_FOLDER}`);
}
...
app.get("/", (req, res) => {
  res.send(`
    ...
    <body>
      ...
      <p>Local folder: <code>${PRODUCER_FOLDER}</code></p>
      <p>Max file size: ${MAX_FILE_MB} MB</p>
...
app.post("/upload", (req, res) => {
  upload.single("video")(req, res, (err) => {
    if (err instanceof multer.MulterError) {
      if (err.code === "LIMIT_FILE_SIZE") {
        return res.status(400).json({
          error: "File too large. Max size is " + MAX_FILE_MB + " MB."
        });
      }
    ...
    (grpcErr, response) => {
        ...
        // For QUEUE_FULL / DUPLICATE / OK, just return 200 with the status in the body
...
app.listen(HTTP_PORT, () => {
  console.log(
    `[PRODUCER ${PRODUCER_ID}] Running at http://localhost:${HTTP_PORT} → forwarding to ${GRPC_ADDR}`
  );
});